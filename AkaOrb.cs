...
namespace CursedTechniques
{
    public class AkaOrb
    {
        ...
        public void Update(Farmer player)
        {
            ...
            if (player.CurrentItem == null || player.CurrentItem.Name != "Aka - Kırmızı") { lifetime = maxLifetime; return; }
        ...
        public void Draw(SpriteBatch sb, Farmer player)
        {
            ...
            Texture2D tex; try { tex = Game1.content.Load<Texture2D>("Mods/CursedTechniques/aka_orb"); } catch { return; }
            float alpha = 0.85f + (float)Math.Sin(bobTimer * 3f) * 0.15f;
            int size = (int)(tex.Width * 2.5f);
            sb.Draw(tex, new Rectangle((int)(screen.X - size/2), (int)(screen.Y - size/2), size, size), null, new Color(1f, 0.3f, 0.3f, alpha), 0f, Vector2.Zero, SpriteEffects.None, 0.9f);
        }
    }
}
